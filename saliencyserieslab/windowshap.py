...
class StationaryWindowSHAP:
    def __init__(self, model, window_len, B_ts, test_ts, B_mask=None, B_dem=None,
        ...
        :param model: A model object that will be used for prediction. The model object must have a method called predict() which produces the model output for a given input
        :param window_len: The length of the window for the algorithm
        :param B_ts: A 3D numpy array of background time series data
        :param test_ts: A 3D numpy array of test time series data
        :param B_mask: A 3D numpy array of background masking data. It is only used for specific models such as GRUD where a masking variable is passed to the model alongside the time series data. (default: None)
        :param B_dem: A 2D numpy array of background demographic data (non-temporal data). It is only used for specific models with both modelities of temporal and non-temporal variables. (default: None)
        :param test_mask: A 3D numpy array of test mask data (default: None)
        :param test_dem: A 2D numpy array of test demographic data (default: None)
        :param model_type: The type of model being used. Set the parameter to 'lstm' when time series data is the only input, pick 'lstm_dem' when input includes both time sereis and demographic (non-termporal) data, and 'grud' when you are using GRUD structure. (default: 'lstm')
        '''
