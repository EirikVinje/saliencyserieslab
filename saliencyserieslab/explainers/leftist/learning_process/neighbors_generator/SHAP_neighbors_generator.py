...
from saliencyserieslab.explainers.leftist.learning_process.neighbors_generator.neighbors_generator import NeighborsGenerator
from saliencyserieslab.explainers.leftist.neighbors import Neighbors
from saliencyserieslab.explainers.leftist.learning_process.utils_learning_process import reconstruct
...
class SHAPNeighborsGenerator(NeighborsGenerator):
    ...
    def _allocate(self,nb_features,nb_neighbors):
        ...
        self.num_subset_sizes = int(np.ceil((nb_features - 1) / 2.0))
        self.num_paired_subset_sizes = int(np.floor((nb_features - 1) / 2.0))
